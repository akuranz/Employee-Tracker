...
-- SELECT roles.dapartment_id, employees.first_name FROM roles LEFT OUTER JOIN employees ON roles.id = employees.role_id UNION ALL SELECT roles.title, departments.name FROM roles RIGHT OUTER JOIN departments ON departments.name = roles.department_id;
